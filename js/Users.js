class Users {
  ...
  changeActiveMember() {
    ...
    if (this.validateAccNum(accNum)) {
      ...
      const newActiveMember = this.members.find(
        (member) => member.accNum === accNum
      );
  ...
  removeMember(member) {
    this.members = this.members.filter(
      (existingMember) => existingMember.accNum !== member.accNum
    );
  }
  ...
  validateAccNum(accNum) {
    if (
      isNaN(accNum) ||
      this.members.find((member) => member.accNum === accNum) === undefined
    ) {
      return false;
    } else {
      return true;
