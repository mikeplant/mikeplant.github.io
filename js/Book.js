class Book {
  ...
  getHTML(optionsArr, item, member) {
    ...
    const options = {
      ...
      userCurrent: () => {
        html += `
					<strong>Title: </strong><span id="item-title" class="item-property">${
            this.title
          }</span>
					<strong>Author: </strong><span id="item-author" class="item-property">${
            this.author
          }</span>
					<strong>Checked Out: </strong><span>${library.getDateString(
            item.dateCheckedOut
          )}</span>
					<strong>Return Due: </strong><span>${library.getDateString(
            item.returnDue
          )}</span>
					`;
        if (
          userHasItem &&
          library.returnIsOverdue(member.getRentedItem(this))
        ) {
          html += `<span class="user-has-span overdue">Return overdue</span>`;
        }
      ...
      userPrevious: () => {
        html += `
            ...
            this.author
					...
					<strong>Rental Date: </strong><span>${library.getDateString(
            item.dateCheckedOut
          )}</span>
					`;
        if (userHasItem) {
