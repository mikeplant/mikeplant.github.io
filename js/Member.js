class Member {
  constructor(
    ...
    joinDate = new Date(),
    lateReturns = 0,
    isBanned = this.isOverLateReturnLimit()
  ) {
    this.accNum = accNum;
    this.name = name;
    this.age = age;
  ...
  getHTML() {
    ...
    let html = `
            ...
            <strong>Member Since: </strong><span>${library.getDateString(
              this.joinDate
            )}</span>`;
  ...
  getRentedItem(item) {
    return this.currentRentals.find(
      (rentedItem) => rentedItem["item"].stockNum === item.stockNum
    );
  }
  ...
  checkInItem(item) {
    if (library.returnIsOverdue(this.getRentedItem(item))) {
      this.handleLateReturn();
    }
    this.currentRentals = this.currentRentals.filter(
      (rentedItem) => rentedItem["item"].stockNum !== item.stockNum
    );
