class ViewHandler {
    ...
    const displayOptions = {
      ...
      allMembers: () => {
        ...
        library.users.members.forEach((member) =>
          mainContent.appendChild(member.getHTML())
        );
      },
      addMember: () => {
      ...
      inStockSearch: () => {
        ...
        this.mainContent.appendChild(
          htmlContent.getSearchHTML("inStockSearch")
        );
        this.getSearchDivListener();
      },
      memberSearch: () => {
  ...
  displayMemberItems(member) {
    if (member) {
      ...
      member.currentRentals.forEach((item) => {
        document
          .querySelector("#current-rentals")
          .appendChild(
            item["item"].getHTML(["userCurrent", "button"], item, member)
          );
      });
      member.previousRentals.forEach((item) => {
        document
          .querySelector("#previous-rentals")
          .appendChild(
            item["item"].getHTML(
              ["userPrevious", "stockQuantity", "button"],
              item,
              member
            )
          );
  ...
  updateItemCard(div, option) {
    ...
    const options = {
      ...
      edit: () => {
        updatedCard = item[0].getHTML(["details", "editButtons"]);
      },
    };
    options[option]();
  ...
  clearModalAndUpdateDisplay = (
    ...
    window.setTimeout(() => {
      if (!isNaN(itemCardElement.id) && itemCardElement.id !== "") {
        if (
          itemCardElement.classList.contains("previous-item") ||
          itemCardElement.classList.contains("current-item")
        ) {
          this.clearUserItemsDisplay();
          this.displayMemberDetails(
            this.prevMember,
      ...
      add: () => {
        ...
        searchItems.forEach((item) => {
          document
            ...
            .appendChild(item.getHTML(["details", "addButton"]));
        });
      },
      edit: () => {
        this.main.appendChild(htmlContent.getSearchResultHTML());
        searchItems.forEach((item) => {
          document
            .querySelector("#search-results")
            .appendChild(item.getHTML(["details", "editButtons"]));
        });
      },
      itemSearch: () => {
        this.main.appendChild(htmlContent.getSearchResultHTML());
  ...
  handleItemCardBtnClick(e) {
    ...
    const item = !isNaN(parseInt(parent.id))
      ? library.getItemByStockNum(parseInt(parent.id))
      : library.getItemByStockNum(parseInt(parent.parentNode.parentNode.id));
    ...
    const buttonAction = {
      ...
      checkin: () => {
        modals.fadeInModal(
          modals.getCheckInModalHTML(item, data.activeMember),
          parent,
          "100"
        );
        data.activeMember.checkInItem(item);
        this.clearModalAndUpdateDisplay(
      ...
      confirm: () => {
        ...
        if (library.validateForm(parent)) {
          ...
          if (div.classList.contains("item-card-modal")) {
            ...
            modals.fadeInModal(
              modals.getManualAddConfirmModal(itemProperties),
              parent.parentNode,
              "100"
            );
            this.clearModalAndUpdateDisplay(
              div.querySelector(".add-book-manual-modal"),
              "2000",
              div.querySelector(".add-book-manual-modal"),
              "2500",
              "main"
  ...
  handleEditItemCardBtnClick(e) {
    ...
    const buttonAction = {
      ...
      delete: () => {
        const itemToRemove = library.getItemByStockNum(
          parseInt(parent.parentNode.parentNode.id)
        );
  ...
  handleMemberCardBtnClick(e) {
    if (!isNaN(e.target.parentNode.id)) {
      const member = library.users.getMemberByAccNum(
        parseInt(e.target.parentNode.id)
      );
  ...
  handleEditMemberClick(e) {
    ...
    let member = library.users.getMemberByAccNum(
      parseInt(document.querySelector("#user-accNum").textContent)
    );
    const buttonAction = {
      edit: () => {
        const userProperties = this.getPropertiesFromUserDiv(parent);
        modals.fadeInModal(modals.getEditMemberModal(), parent, "100");
        this.populateEditMemberForm(
      ...
      confirm: () => {
        ...
        if (library.validateForm(parent)) {
          ...
          modals.fadeInModal(
            modals.getEditMemberConfirmModal(member),
            parent,
            "100"
          );
          setTimeout(() => {
  ...
  handleAddMemberBtnClick(e) {
    ...
    if (library.validateForm(form)) {
      ...
      modals.fadeInModal(
        modals.getAddMemberConfirmModal(userProperties),
        document.querySelector(".add-member-form-div"),
        100
      );
      this.clearModalAndUpdateDisplay(
        document.querySelector(".item-card-modal"),
        2000,
        document.querySelector(".item-card-modal"),
        2100
      );
  ...
  handleSearchEvent(searchTerm, event) {
    ...
    const eventOptions = {
      itemSearch: () => {
        viewHandler.updateSearchResultsDisplay(
          "itemSearch",
          library.getItemsBySearchTerm(searchTerm)
        );
      },
      inStockSearch: () => {
        viewHandler.updateSearchResultsDisplay(
      ...
      memberSearch: () => {
        viewHandler.updateSearchResultsDisplay(
          "memberSearch",
          library.users.getMembersBySearchTerm(searchTerm)
        );
      },
    };
  ...
  handleRemoveMemberClick(parent, member) {
    ...
    if (!member.userHasAnyItem() && !member.isActive()) {
      modals.fadeInModal(
        modals.getConfirmRemoveMemberModal(member),
        parent,
        "100"
      );
    } else {
      modals.fadeInModal(
        modals.getRemoveMemberDisallowedModal(),
        parent,
        "100"
      );
      this.clearModalAndUpdateDisplay(
  ...
  getPropertiesFromItemCard(element) {
    ...
    children
      .filter((child) => child.classList.contains("item-property"))
      .forEach(
        (property) =>
          (itemProperties[property.id.split("-")[1]] =
            property.tagName === "SPAN" ? property.textContent : property.value)
      );
    if (itemProperties.inStock)
      itemProperties.inStock = parseInt(itemProperties.inStock);
    ...
    children
      ...
      .forEach((input) => {
        if (itemProperties[input.id.split("-")[1]]) {
          element.querySelector(`#${input.id}`).value =
            itemProperties[input.id.split("-")[1]];
        }
      });
  }
  ...
  getPropertiesFromUserDiv(element) {
    ...
    children
      .filter((child) => child.classList.contains("user-property"))
      .forEach(
        (property) =>
          (userProperties[property.id.split("-")[1]] =
            property.tagName === "P" ? property.textContent : property.value)
      );
    ...
    children
      ...
      .forEach((input) => {
        if (userProperties[input.id.split("-")[1]]) {
          element.querySelector(`#${input.id}`).value =
            userProperties[input.id.split("-")[1]];
        }
      });
  }
